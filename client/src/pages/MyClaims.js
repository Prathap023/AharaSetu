...
function MyClaims() {
 ...
 useEffect(() => {
  ...
  const fetchClaims = async () => {
    try {
      const res = await axios.get('https://aharasetu-backend.onrender.com/api/food/my-claims', {
  ...
  const handlePickedUp = async (id) => {
    try {
      await axios.put(`https://aharasetu-backend.onrender.com/api/food/volunteer-pickedup/${id}`, {},
        { headers: { Authorization: `Bearer ${token}` } });
